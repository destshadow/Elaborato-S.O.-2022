        ...
        uint32_t **mat_need, size_t n, size_t m)
{
    ...
    while (arr_ne(finish, all_true, n))
    {
        ...
        {
           work = arr_add(work, mat_alloc[i], m); //metto dentro a work le risorse che ho allocato
           finish[i] = 1;
        }
    }
        ...
        size_t n, size_t m)
{
    ...
    mat_need[task_i] = arr_sub(mat_need, req_vec, m);
    mat_alloc[task_i] = arr_add(mat_alloc[task_i], req_vec, m);

    // Check safe state
    if (! state_safe(arr_available, mat_need, mat_alloc, n, m) )
    {
